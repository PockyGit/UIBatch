...
public class UIAssistantWindow : EditorWindow
{
    ...
    private Color[] m_colors = new Color[6]
        {Color.red, Color.yellow, Color.blue, Color.green, Color.black, Color.white};
    ...
    private void ShowCatchUI()
    {
        if (m_selectObj != null)
        {
            GUILabelType(TextAnchor.UpperLeft);
            GUILayout.Space(2);
            GUILayout.Label(m_treeRootNode == null
                ? "Result: "
                : string.Format("Result: {0}({1})", m_treeRootNode.batchCount, m_treeRootNode.maskBatchCount));
            GUILayout.Space(2);

            m_ScrollPos = GUI.BeginScrollView(new Rect(10, 140, 800, position.height - 160), m_ScrollPos,
                new Rect(0, 0, m_treeRootNode.RecursiveSize.x, m_treeRootNode.RecursiveSize.y), true, true);
